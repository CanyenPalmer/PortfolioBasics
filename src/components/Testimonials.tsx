...
export type Testimonial = {
  app: string;          // e.g., "Best Bet NFL"
  name?: string;        // optional person/source
...
type Props = {
  heading?: string;
  items: Testimonial[];
  cols?: 2 | 3; // grid columns on desktop
};
  ...
  return (
    <section id="testimonials" className="section-wrap">
      <div className="hud-panel">
        ...
        >
          {items.map((t, i) => (
            <FlipCard key={i} t={t} delay={i * 0.05} />
          ))}
        </div>
      </div>
    </section>
...
/** Large flip card with enough interior padding so text never overflows. */
      ...
      >
        ...
        <div className="flip-face flip-back rounded-2xl ring-1 ring-white/12 bg-white/[0.05] p-6 md:p-7 min-h-[240px] lg:min-h-[280px]">
          <div className="h-full flex flex-col">
            ...
            <div className="mt-auto pt-4 text-right text-xs text-white/65">
              Click to flip back
            </div>
          </div>
        </div>
