...
type Section = {
  id: string;      // must match the section's element id on the page (e.g., "experience")
  label: string;   // label in the tab bar (e.g., "Experience")
};

type Props = {
  sections?: Section[];
  resumeHref?: string;     // e.g., "/Canyen_Palmer_Resume.pdf"
  linkedinHref?: string;   // e.g., "https://www.linkedin.com/in/yourname"
  githubHref?: string;     // e.g., "https://github.com/yourname"
  /** Optional: override the signature text on the far left */
  signature?: string;      // default: "Canyen Palmer"
};
...
export default function VscodeTopBar({
  ...
  // Observe sections to update active tab as user scrolls.
  React.useEffect(() => {
    const observers: IntersectionObserver[] = [];
    const marginTop = 96; // offset so header doesn't cover titles
    ...
    sections.forEach((s) => {
      ...
      const io = new IntersectionObserver(
        ...
        {
          root: null,
          // Start “active” when the section top is ~96px below the viewport top
          rootMargin: `-${marginTop}px 0px -66% 0px`,
          threshold: 0.1,
        }
      );
      io.observe(el);
  ...
  const onTabClick = (e: React.MouseEvent, id: string) => {
    ...
    if (!el) return;
    const headerOffset = 72; // height of the sticky bar
    const rect = el.getBoundingClientRect();
    const offset = window.scrollY + rect.top - headerOffset - 8; // tiny pad
    ...
    >
      <div className="mx-auto max-w-6xl px-4 md:px-6">
        <div className="h-14 flex items-center gap-3">
          {/* Left cluster: mac traffic lights + signature */}
          ...
          <nav
            className="
              mx-2 flex-1 min-w-0 overflow-x-auto
                ...
                return (
                  <li key={s.id} className="shrink-0">
                    <a
                      ...
                      className={`
                        group inline-flex items-center px-3 py-1.5 rounded-md
