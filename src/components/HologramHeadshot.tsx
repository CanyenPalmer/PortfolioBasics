...
type Props = {
  ...
  // NEW: shift the image inside the frame without moving the frame
...
export default function HoloHeadshotAuto({
  ...
  // slower bars by default
  waveBars = true,
  waveBarCount = 3,
  wavePeriodMs = 3600,       // was 2600 → slower sweep
  waveBarHeightPct = 12,
  waveIntensity = 1.0,
  ...
  imageYOffsetPx = -10,      // lift image a bit inside the frame
}: Props) {
  const uid = React.useId(); // unique id to namespace SVG filters
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        // dynamic import so SSR doesn’t choke (deps must be installed locally)
  ...
  const drawCover = React.useCallback((dest: HTMLCanvasElement, yOffsetPxLocal: number) => {
    ...
    if (srcAspect > dstAspect) {
      ...
      // map container y-offset to source crop in this branch
  ...
  const bars = React.useMemo(() => {
    ...
    for (let i = 0; i < n; i++) {
      arr.push({
        ...
        speedScale: 0.95 + Math.random() * 0.15, // narrower variance for smoother pace
  ...
  return (
    ...
    >
      ...
      {/* Horizontal Wavy Bars: tinted strip + CLIPPED, WARPED canvas clone with SVG ripple */}
      {waveBars &&
        ready &&
        bars.map((b, i) => {
          ...
          return (
            <React.Fragment key={i}>
              {/* SVG ripple filter definition (unique per bar) */}
                ...
                <filter id={filterId}>
                  ...
                  >
                    <animate
                      ...
                      dur={`${Math.max(1.8, (sweepMs / 1000) * 0.9)}s`}
                      values="0.010 0.10;0.006 0.16;0.010 0.10"
              ...
              {/* Warped clone of the image, clipped to the same bar shape with SVG ripple */}
              <div
                aria-hidden
              ...
              >
                <canvas
                  ...
                  style={{
                    ...
                    // local transforms enhance the ripple impression
                    animation: `
                    ...
                    `,
                    filter: `url(#${filterId}) brightness(${1 + 0.2 * waveIntensity}) saturate(${1 + 0.2 * waveIntensity})`,
                  }}
                />
              </div>
            </React.Fragment>
