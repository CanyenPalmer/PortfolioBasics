        ...
        <div className="grid md:grid-cols-[1.1fr_.9fr] gap-10 items-center">
          <div>
            <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
              className="text-4xl md:text-6xl font-extrabold tracking-tight">Canyen Palmer</motion.h1>
            <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .1, duration: .6 }}
              className="mt-3 text-lg md:text-xl text-white/80">Data Scientist & Google-Certified Data Analytics Professional</motion.p>
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .2, duration: .6 }} className="mt-6">
              <TypeLine />
            </motion.div>
          </div>
          <motion.div initial={{ opacity: 0, scale: .98 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: .15, duration: .6 }}
            ...
            <svg viewBox="0 0 520 520" className="w-full h-auto drop-shadow-[0_0_24px_rgba(0,229,255,0.35)]">
              ...
              <motion.rect x="274" y="318" width="24" height="92" rx="10" className="fill-[#121a26] stroke-[#00e5ff]" strokeWidth="2"
                animate={{ y: [0, 1.5, 0] }} transition={{ duration: 2.2, repeat: Infinity }} />
              <motion.circle cx="420" cy="100" r="6" className="fill-[#00e5ff]" animate={{ opacity: [0.25, 1, 0.25] }} transition={{ duration: 1.6, repeat: Infinity }} />
              <motion.circle cx="96" cy="420" r="6" className="fill-[#ff3bd4]" animate={{ opacity: [0.25, 1, 0.25] }} transition={{ duration: 1.8, repeat: Infinity, delay: .2 }} />
              <motion.circle cx="440" cy="360" r="6" className="fill-white" animate={{ opacity: [0.25, 1, 0.25] }} transition={{ duration: 1.4, repeat: Infinity, delay: .1 }} />
            </svg>
          </motion.div>
        </div>
      </div>
...
function TypeLine() {
  return (
    <div className="font-mono text-[15px] md:text-[17px] text-white/90">
      ...
      <span className="typewriter">Turning data into decisions through science, code, and storytelling.</span>
      <span className="caret" aria-hidden />
      <p className="mt-2 text-white/60 text-sm">Clean pipelines → robust models → clear dashboards.</p>
    </div>
  );
}
